<?php foreach ($actions['loyalties'] as $row):?>
    <tr>
        ...
        <td class="t-condition"><div>
            <h2>
                <?php echo $row->loyalty->amount; ?><i class="icon rub">&#xe019;</i>
            </h2>
            <p>
                <?php echo $row->loyalty->getRulesDesc(); ?>
            </p>
        </div></td>
        ...
        <td>
            <div>
                <?php echo date('d.m.y', strtotime($row->loyalty->start_date)).' - '.date('d.m.y', strtotime($row->loyalty->stop_date));?>
            </div>
        </td>
        <td class="t-bonus">
            <div>
            ...
            <ul class="pagination">
                <?php for ($i=1; $i <= $actions['pagination']['pages']; $i++):?>
                    <li <?php echo ($actions['pagination']['current'] == $i)?'class="current"':''; ?>>
                        <a ng-click="getSpecialActions(<?php echo $wallet->id.', '.$i; ?>)">
                            <?php echo $i; ?>
                        </a>
                    </li>
                <?php endfor;?>
