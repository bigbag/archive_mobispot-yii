 ...
 * modified by Antonio Ramirez http://www.ramirezcobos.com
 * 10.26.2011 Enhanced plugin to work with multiple instances
 * 10.27.2011 Enhanced plugin to work with internationalization -with Yii extension
 * 10.28.2011 Enhanced plugin for collision detection
 *
...
jQuery.fn.daterangepicker = function (settings) {
    ...
    var id = rangeInput.attr('id');
    var rid = 'date-range-picker-' + id;
    var sid = 'date-range-picker-start-' + id;
    var eid = 'date-range-picker-end-' + id;
    //defaults
    ...
    var datepickerOptions = {
        onSelect:function (dateText, inst) {
            ...
            if (!$rend.data('datepicker')) return;
            var _dt_start = $rstart.length && $rstart.datepicker ? $rstart.datepicker('getDate') : false;
            var _dt_end = $rend.length && $rend.datepicker ? $rend.datepicker('getDate') : false;

            if (rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active') && inst.id != eid) {
                _dt_end = _dt_start;
                $rend.datepicker('setDate', _dt_start);
            }
            else if (inst.id == sid)
                $rend.datepicker('option', 'minDate', _dt_start);
            else if (rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active') && inst.id == eid)
                return;

            var rangeA = fDate(_dt_start);
            var rangeB = fDate(_dt_end);
    ...
    //build picker and 
    var rp = jQuery('<div id="' + rid + '" class="ui-daterangepicker ui-widget ui-helper-clearfix ui-widget-content ui-corner-all"></div>');
    ...
    function findPos() {
        var $window = $(window),
            wide = $window.width() + $window.scrollLeft(),
            height = $window.height() + $window.scrollTop(),
            offset = rangeInput.offset();

        var pos = [offset.top <= $window.scrollTop(), wide <= offset.left + rp.width(), height <= offset.top + rp.height(), $window.scrollLeft() >= offset.left];
        var top = (offset.top + rangeInput[0].offsetHeight);
        var left = offset.left;

        return {top:(pos[2] ? top - (rangeInput.height() + rp.height() + rangeInput[0].offsetHeight) : top), left:(pos[3] ? $window.scrollLeft() + 1 : left)};
    }

    //show, hide, or toggle rangepicker
    function showRP() {
        var pos = findPos();
        rp.parent().css('top', pos.top);
        rp.parent().css('left', pos.left);
    ...
    //picker divs
    var rpPickers = jQuery('<div class="ranges ui-widget-header ui-corner-all ui-helper-clearfix"><div id="' + sid + '" class="range-start"><span class="title-start">Start Date</span></div><div id="' + eid + '" class="range-end"><span class="title-end">End Date</span></div></div>').appendTo(rp);
    ...
    if (options.arrows && rangeInput.size() == 1) {
        ...
        jQuery(this)
            ...
            .parent().find('a').click(function () {
                ...
                $('#' + sid).datepicker('setDate', dateA.add({milliseconds:diff}));
                $('#' + eid).datepicker('setDate', dateB.add({milliseconds:diff}));
